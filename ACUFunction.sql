...
RETURN (
	...
	*********************************************************/
	...
	InitProductsCTE AS (
		...
		SELECT
			...
			,OPENDATE AS 'ProductOpenDate'
			,CLOSEDATE AS 'ProductCloseDate'
			,NULL AS 'ProductTerm'
		...
		SELECT
			Loan.ParentAccount AS 'AccountParentAccount'
			,'Loan' AS 'AccountType'
			,Loan.Type AS 'AccountType'
			,Loan.ID AS 'AccountID'
			,Loan.CollateralCode AS 'LoanCollateralCode'
			,Loan.DESCRIPTION AS 'AccountDescription'
			,Loan.OpenDate AS 'AccountOpenDate'
			,Loan.CloseDate AS 'AccountCloseDate'
	...
	*********************************************************/
	...
	FROM
		...
		--RIGHT OUTER JOIN MembersPromotionsCTE
		--	ON	MembersCTE.UniqueID = MembersPromotionsCTE.PromotionsUniqueID 
				--AND (@Lens LIKE 'Members/Promotions')
		--FULL OUTER JOIN TellerTransactionsCTE
		--	ON MembersCTE.AccountNumber = TellerTransactionsCTE.TransactionParentAccount AND (@Lens = 'Transactions')
		/* This transaction table cannot be used yet. Base table not optimized
		LEFT OUTER JOIN InitTransactionsCTE
			ON ( (SharesCTE.ShareParentAccount = InitTransactionsCTE.PARENTACCOUNT AND SharesCTE.ShareID = InitTransactionsCTE.PARENTID)
				OR (LoansCTE.LoanParentAccount = InitTransactionsCTE.PARENTACCOUNT AND LoansCTE.LoanID = InitTransactionsCTE.PARENTID) )
				AND (@Lens = 'Transactions')
		*/
